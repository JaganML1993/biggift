...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="row">
        ...
        <div class="col-xxl">
            <div class="card mb-4">
                <div class="card-body">
                    <form action="{{ route('products.ecom.update') }}" method="POST" enctype="multipart/form-data" >
                        @csrf
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-control" name="category_id" required>
                                    ...
                                    @foreach ($category as $item)
                                        <option value="{{$item->id}}" @if ($product_edit->category_id == $item->id) selected @endif>{{$item->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-control" name="subcategory_id" required>
                                    ...
                                    @foreach ($subCategory as $item)
                                        <option value="{{$item->id}}" @if ($product_edit->subcategory_id == $item->id) selected @endif>{{$item->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-control" name="related_products[]" multiple="multiple">
                                    @php
                                        $short_description_array = explode(",",$product_edit->related_products);
                                    @endphp
                                    @foreach ($products as $item)
                                    <option value="{{$item->id}}" @if(in_array($item->id,$short_description_array)) selected @endif>{{$item->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                @if (count($product_images))
                                    ...
                                    <a href="{{ url('').$item->path }}" target="_blank"><img src="{{ url('').$item->path }}" style="width:20%; margin-top: 20px;"></a><a style="margin-left: -10px; margin-top: -35px; color: #fff;" class="btn btn-danger btn-xs" onclick="return confirm('Are you sure to remove this image?')" href="{{ route('products.ecom.delete_image', ['id' => $item->id]) }}">X</a>
                                    @endforeach
                                @endif
                            </div>
